...
function sendThis(){
    rsync -avzh --no-owner --no-group --chmod=Du=rwx,Dg=rwx,Do=rx,Fu=rwx,Fg=rw,Fo=r  --exclude={.env,*.sh,*.sql,*.vim,*.zip,.git/*} --progress "${localprj}" root@136.243.160.136:${remotedir}
    }
function recvThis(){
    echo "recv"
}
