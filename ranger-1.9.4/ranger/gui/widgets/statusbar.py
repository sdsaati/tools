...
class StatusBar(Widget):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, win, column=None):
        ...
        self.settings.signal_bind(
            "setopt.display_size_in_status_bar", self.request_redraw, weak=True
        )
    ...
    def _draw_message(self):
        self.win.erase()
        self.color("in_statusbar", "message", self.msg.bad and "bad" or "good")
    ...
    def _get_left_part(
        ...
        if (
            ...
            and self.column.target is not None
        ...
        if target.is_link:
            ...
            jalali = jdatetime.datetime.fromtimestamp(
                timee.mktime(localtime(stat.st_mtime))
            )
            left.add(jalali.strftime("%Y-%m-%d") + ", " + date, "mtime")
            # left.add(date, "mtime")

        directory = (
            target
            if target.is_directory
            else target.fm.get_directory(os.path.dirname(target.path))
        )
        if directory.vcs and directory.vcs.track:
            ...
            if directory.vcs.rootvcs.head:
                left.add_space()
                left.add(
                    directory.vcs.rootvcs.head["date"].strftime(self.timeformat),
                    "vcsdate",
                )
                left.add_space()
                summary_length = self.settings.vcs_msg_length or 50
                left.add(
                    directory.vcs.rootvcs.head["summary"][:summary_length], "vcscommit"
                )
    ...
    def _get_right_part(
        ...
        if target.marked_items:
            ...
            right.add(
                str(target.pointer + 1) + "/" + str(len(target.files)) + "  ", base
            )
            if max_pos <= 0:
